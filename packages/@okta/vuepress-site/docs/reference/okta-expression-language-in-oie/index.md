---
title: Expression Language In Okta Identity Engine
meta:
  - name: description
...
This document details the features and syntax of the Okta Expression Language used in Okta Identity Engine. Expressions being used outside of Okta Identity Engine should continue using the features and syntax of [the legacy Okta Expression Language](/docs/reference/okta-expression-language/). This document will be updated over time as new capabilities
are added to the language. Okta expression language is based on [SpEL](http://docs.spring.io/spring/docs/3.0.x/reference/expressions.html) and uses a subset of functionalities offered by SpEL.

## Unsupported Features

The following operators and functionalities offered by SpEL are not supported in Okta expression language:
...
> **Note:** For the following expression examples, assume that *user* has the following attributes in Okta.
...
**Note:**  All these functions can be called on ISO 3166-1 2-character country codes (Alpha 2), 3-character country codes (Alpha 3), numeric country codes, and country code Strings.
The functions return the output in the format specified by the function name.
...
Value: Specifies a list matching values which can be exact values or a regex pattern (Only start with is supported)
...
> **Note:** For the following expression examples, assume that current date time is `2015-07-31T17:18:37.979Z`.
...
* User attributes used in expressions can contain only available User attributes.
